...
const Rentals = () => {
  ...
  useEffect(() => {
    async function fetchRentalsList() {
      ...
      setRentalsList(queryResults);
      
      /*
      for (
        var arr = [], dt = new Date(searchFilters.checkIn);
        dt <= searchFilters.checkOut;
        dt.setDate(dt.getDate() + 1)
      ) {
        arr.push(new Date(dt).toISOString().slice(0, 10)); // yyyy-mm-dd
      }
      
      const checkBookingsAbi = [ 
        {
        "inputs": [
          {
      ...
      queryResults.forEach((e) => {
        const {
          ...
          abi: checkBookingsAbi,
          params: {
            id: e.attributes.uid_decimal.value.$numberDecimal,
            newBookings: arr,
        ...
        if(data) {
          results.push(e);
          cords2.push({ lat: e.attributes.lat, lng: e.attributes.long });
        }
      });
      */
    }
  ...
  const bookRental = async function (start, end, id, dayPrice, guests) {
    ...
    let options = {
      ...
      msgValue: String(dayPrice * arr.length * guests),
    }
    console.log(arr);
